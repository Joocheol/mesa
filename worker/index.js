...
const json = (value, status = 200, headers = {}) =>
  new Response(JSON.stringify(value), { status, headers: { "content-type": "application/json; charset=utf-8", "cache-control": "no-store", ...headers } });
...
async function isInstructor(request, env) {
  ...
  return (await hmac(expires, env.SESSION_SIGNING_SECRET)) === signature;
}
...
// Round 3 is the sealed exam window: it starts closed and the instructor opens it on the day.
...
function asset(pathname) {
  ...
  return new Response(bytes, { headers: { "content-type": mime(path), "cache-control": cache } });
}
...
export async function handleApi(request, env) {
  ...
  if (path === "/api/instructor/login" && request.method === "POST") {
    ...
    if ((await sha256(String(data.password || ""))) !== expected) return json({ error: "강사 비밀번호가 올바르지 않습니다." }, 403);
  ...
  if (path === "/api/score" && request.method === "POST") {
    ...
    if (!classCode || !SCORE_ACTIVITIES.has(activityId) || !voterId || !teamName || !Number.isFinite(value)) return json({ error: "점수 입력값을 확인하세요." }, 400);
...
export default {
  async fetch(request, env) {
    ...
    if (api) return api;
    return asset(new URL(request.url).pathname) || new Response("Not found", { status: 404 });
  },
};
